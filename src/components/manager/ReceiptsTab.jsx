      ...
      <div className="card overflow-hidden">
        ...
        {filtered.map((r) => (
          <div key={r.id} className="grid grid-cols-[1fr_2fr_2fr_1fr_1fr_1fr] px-6 py-4 border-b border-gray-50 hover:bg-gray-50 transition-colors cursor-pointer last:border-0 items-center">
            <span className="text-sm font-mono font-medium text-primary">{r.id}</span>
            <span className="text-sm font-medium text-navy">{r.supplier}</span>
            <span className="text-sm text-navy">{r.product}</span>
            <span className="text-sm font-semibold text-green-700">{r.qty}</span>
