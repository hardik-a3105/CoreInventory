...
export default function TransferTasks() {
  ...
  const tasks = (operations || []).filter(t => t.type === 'TRANSFER' && t.status === 'PENDING')

  return (
    <div className="card">
      <div className="px-6 py-4 border-b border-gray-100">
        <h3 className="text-base font-bold text-navy">Transfers to Do</h3>
        <p className="text-sm text-muted mt-0.5">Move stock between locations</p>
      </div>
      
      {tasks.length === 0 ? (
        <div className="px-6 py-12 text-center text-muted text-sm italic">No pending transfers.</div>
      ) : (
        tasks.map((t) => (
          <div key={t.id} className="flex items-center gap-4 px-6 py-5 border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2 mt-1.5">
                <span className="text-xs font-semibold bg-indigo-50 text-indigo-700 px-2 py-1 rounded-lg">From: {t.location}</span>
                <svg className="w-3 h-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" />
                </svg>
                <span className="text-xs font-semibold bg-green-50 text-green-700 px-2 py-1 rounded-lg">To: {t.destination || 'N/A'}</span>
              </div>
            </div>
            ...
            <button 
              onClick={() => updateOperationStatus(t.id, 'COMPLETED')}
              className="text-xs font-semibold bg-primary text-white px-4 py-2 rounded-lg border-none cursor-pointer hover:bg-primary-dark transition-colors"
            >
              Complete
            </button>
          </div>
