...
export default function DeliveriesTab() {
  const { deliveries, fetchDeliveries, loading } = useInventoryStore()
  ...
  if (loading && deliveries.length === 0) {
    return <div className="p-6 text-center text-muted">Loading deliveries...</div>
  }
      ...
      <div className="card overflow-hidden">
        ...
        {filtered.map((d) => (
          <div key={d.id} className="grid grid-cols-[1fr_2fr_2fr_1fr_1fr_1fr] px-6 py-4 border-b border-gray-50 hover:bg-gray-50 transition-colors cursor-pointer last:border-0 items-center">
            <span className="text-sm font-mono font-medium text-primary">{d.deliveryNumber || d.id}</span>
            <span className="text-sm font-medium text-navy">{d.customer?.name || d.customerName || '−'}</span>
            <span className="text-sm text-navy">{d.product?.name || d.productName || '−'}</span>
            <span className="text-sm font-semibold text-red-600">{d.quantity || d.qty || '−'}</span>
