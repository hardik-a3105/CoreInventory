...
export default function TransfersTab() {
  const { transfers, fetchTransfers, loading } = useInventoryStore()
  ...
  if (loading && transfers.length === 0) {
    return <div className="p-6 text-center text-muted">Loading transfers...</div>
  }
      ...
      <div className="card overflow-hidden">
        ...
        {filtered.map((t) => (
          <div key={t.id} className="grid grid-cols-[1fr_1.5fr_1.5fr_2fr_1fr_1fr] px-6 py-4 border-b border-gray-50 hover:bg-gray-50 cursor-pointer last:border-0 items-center">
            <span className="text-sm font-mono font-medium text-primary">{t.transferNumber || t.id}</span>
            <span className="text-sm font-medium text-navy">{t.fromWarehouse?.name || t.from || '−'}</span>
            <span className="text-sm font-medium text-navy">{t.toWarehouse?.name || t.to || '−'}</span>
            <span className="text-sm text-navy">{t.product?.name || t.product || '−'}</span>
            <span className="text-sm font-semibold text-blue-700">{t.quantity || t.qty || '−'}</span>
